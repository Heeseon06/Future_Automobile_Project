...
> **YOLO 기반 객체 탐지와 StrongSORT 추적으로 도로 객체를 실시간으로 탐지·추적하고,
...
**객체 탐지와 StrongSORT 기반 객체 추적 시스템**을 구현했습니다.

먼저 **YOLOv7-tiny**에 커스텀 데이터셋(Open Images V7 오토바이 1805장, 주행 영상 추출 100장)을 학습시켜
MS COCO만으로 사전학습된 모델과 탐지 성능을 비교했습니다.

이후 추적 파이프라인에서는 Jetson Nano에서 동작 가능한 경량 모델인 **YOLOv5s**를 객체 탐지기로 사용해
동일한 커스텀 데이터셋을 학습시키고, 학습된 모델을 **Jetson Nano에 이식하여 실제 주행 환경에서 실시간 동작을 검증**했습니다.
...
- **YOLOv7-tiny 기반 탐지 성능 개선**
  - 커스텀 데이터셋을 추가 학습해 오토바이 탐지 성능 개선
  - MS COCO 사전학습 모델과 성능 비교·검증

- **YOLOv5s + StrongSORT 기반 객체 추적**
  - Jetson Nano에서 동작 가능한 경량 탐지기(YOLOv5s)를 StrongSORT와 연결
...
- YOLOv7-tiny 객체 탐지 모델 커스텀 데이터셋 추가 학습
- 사전학습 모델 대비 **오토바이 클래스 AP(Average Precision) 약 0.5%p 향상**
- YOLOv5s를 탐지기로 사용한 StrongSORT 기반 객체 추적 기능 구현
...
**AI / Computer Vision**
- YOLOv7-tiny (탐지 성능 개선 실험)
- YOLOv5s (추적 파이프라인 탐지기)
- StrongSORT
- OpenCV
...
- MS COCO 사전학습 모델 대비 오토바이 클래스 **AP(Average Precision) 약 0.5%p 향상**
  (Pascal VOC 2012 오토바이 574장으로 테스트)
